-- Migration to ensure ranking schema and RPC exist
-- This matches what is in update_schema_stats.sql but ensures it's in a migration file
...
begin
  ...
  select 
    ...
    -- Custom score calculation (Vitória=5, Empate=2, Gol=3, Assist=2)
    (
      count(case when m.winner_team = mp.team then 1 end) * 5 + 
      count(case when m.winner_team = 'Draw' then 1 end) * 2 +
